...
readonly CONFIG_FILE="$HOME/.config/hypr/workspaces.lua"
    ...
    while ((attempt < max_attempts)); do
        if hyprctl monitors -j &>/dev/null; then
            ...
            if ((monitor_count > 0)); then
                log_info "✓ Hyprland ready with $monitor_count monitor(es)"
                return 0
            fi
        fi
...
add_source_to_config() {
    local hyprland_lua="$HOME/.config/hypr/hyprland.lua"
    local backup_file="${hyprland_lua}.backup-$(date +%Y%m%d-%H%M%S)"

    if [[ ! -f "$hyprland_lua" ]]; then
        log_error "hyprland.lua not found; cannot add workspace loader automatically"
        return 1
    fi

    cp "$hyprland_lua" "$backup_file"

    {
        echo ""
        echo "-- Workspace multi-monitor (auto-generated by workspace-manager.sh)"
        echo "local workspaces = loadfile(os.getenv(\"HOME\") .. \"/.config/hypr/workspaces.lua\")"
        echo "if workspaces then workspaces() end"
    } >>"$hyprland_lua"

    log_info "✓ Added to hyprland.lua"
    log_info "✓ Backup created: $backup_file"
}

# --- Modular Initialization ---
init_monitors() {
    log_info "🚀 Initializing workspaces per monitor..."
    log_info ""
    ...
    log_info "📊 Detected monitors: $monitor_count"

    local temp_config="/tmp/hyprland-workspace-rules-$$.lua"
...
HEADER
    ...
    for idx in "${!monitor_array[@]}"; do
        ...
        log_info "  Monitor $idx: $monitor → base workspace $base_ws"

        echo "hl.workspace_rule({ workspace = \"$base_ws\", monitor = \"$monitor\", default = true })" >>"$temp_config"

        local ws_start=$((idx * OFFSET_SIZE + 1))
        for offset in $(seq 0 9); do
            local ws=$((ws_start + offset))
            echo "hl.workspace_rule({ workspace = \"$ws\", monitor = \"$monitor\" })" >>"$temp_config"
        done
    done
    ...
    if check_source_exists; then
        log_info "✓ Already configured in hyprland.lua"
    else
        log_info ""
        log_info "📝 Add this loader to your hyprland.lua:"
        log_info "    local workspaces = loadfile(os.getenv(\"HOME\") .. \"/.config/hypr/workspaces.lua\")"
        log_info "    if workspaces then workspaces() end"
        log_info ""
        ...
        if [[ -t 0 ]]; then
            ...
            log_info "⚠ Running in non-interactive mode, add manually."
        fi
    fi
    ...
    for idx in "${!monitor_array[@]}"; do
        ...
        log_info "  Moving $mon → workspace $ws"
...
status() {
    ...
    [[ -f "$CONFIG_FILE" ]] && echo "  ✓ Exists" || echo "  ✗ Missing (run --init)"

    echo ""
    echo "Source in hyprland.lua:"
    check_source_exists && echo "  ✓ Configured" || echo "  ✗ Not configured (run --init)"
}
