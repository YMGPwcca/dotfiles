...
setup_monitors() {
    ...
    if [[ ! -f "$MONITORS_FILE" ]]; then
        # Create file with generic Hyprland Lua configuration
...
# =============================================================================
setup_workspaces() {
    log_info "Configuring workspaces.lua..."

    local WORKSPACES_FILE="$HYPR_CONFIG_DIR/workspaces.lua"
...
-- =============================================================================
EOF
        log_info "  Created: workspaces.lua (will be populated automatically)"
    else
        log_warn "  Skipping (already exists): workspaces.lua"
    fi
}
...
run_hyprland_main() {
    ...
    echo "  - ~/.config/hypr/workspaces.lua"
